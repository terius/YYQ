...
km.maingrid = {
    ...
    init: function () {
        this.jq = $("#maingrid").datagrid({
            ...
            method: 'get',
            idField: 'Id',
            queryParams: { _t: com.settings.timestamp(), Code: "" },
            ...
            columns: [[
                { field: 'Code', title: '库位', width: 120, align: 'left', sortable: true },
    ...
    search_data: function () {
        var code = com.trim($("#SearchCode").val());
        // var shelfcode = com.trim($("#shelfCode").val());
        this.reload({ Code: code });
    },
...
km.toolbar = {
    do_add: function () {
        ...
        km.template.jq_add.dialog_ext({
            title: '新增库位种类', iconCls: 'icon-standard-add',
    ...
    do_edit: function () {
        ...
        km.template.jq_add.dialog_ext({
            title: '编辑库位【' + sRow.Code + '】', iconCls: 'icon-standard-edit',
            onOpenEx: function (win) {
                // win.find('#UnitTypeCode').combobox('reload', '/Common/GetUnitSelectList');
                // win.find('#ShelfId').combobox('reload', '/Common/GetShelfSelectList');
    ...
    do_delete: function () {
        ...
        com.message('c', ' <b style="color:red">确定要删除库位【' + sRow.Code + '】吗？ </b>', function (b) {
            if (b) {
                com.ajax({
                    url: km.model.urls["delete"], data: { Id: sRow.Id }, success: function (result) {
                        //layer.msg(result.emsg); 
                        if (result == "") {
                            com.message('s', "删除库位【" + sRow.Code + "】成功");
                            km.maingrid.reload();
                        } else {
                            com.message('e', result);
                        }
